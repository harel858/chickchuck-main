...
function NavBarItem({ title, link, icon }: NavItemProps) {
  ...
  return (
    <li
      className={`flex justify-start gap-8 w-full group hover:bg-opacity-10 hover:bg-white ${
        isActive && `bg-opacity-10 bg-white`
      } cursor-pointer p-4 my-0 transition-all ease-in duration-200`}
      onClick={() => router.push(link)}
    >
      <div
        className={`text-white self-center transition-all ease-in duration-200 group-hover:scale-125 ${
          isActive && `scale-125`
        } text-xl`}
      >
        {icon}
